...
> **세컨드 브레인 확장(예정).** 이 에이전트는 향후 `knowledge/user-brain.md`의 `design`·`global` scope 규칙과 선호 프로파일을 읽어 사용자의 디자인 취향(예: 폰트 시스템, 저작권 이미지 금지, 라이트 테마 우선)을 학습·적용하도록 확장된다. 디자인 피드백에서 `🧠 규칙 후보`를 제안형으로 감지·등록하는 전체 루프 연결은 후속 작업이며, 현재 파일에는 이 **확장 훅만 명시**한다. (루프 메커니즘은 `sermon-storyline-architect`의 「세컨드 브레인 — 피드백 루프」를 동일하게 따른다.)
